...
@jax.jit
def binary_cross_entropy_loss(labels, predictions):
    epsilon = 1e-15
    losses = []

    for l, p in zip(labels, predictions):
        y_pred = jnp.clip((1-p)/2, epsilon, 1 - epsilon)
        l_binary = (1-l)/2
        value = l_binary*jnp.log(y_pred) + (1-l_binary)*jnp.log(1-y_pred)
        losses.append(value)
    loss = -1*jnp.array(losses)
...
def cost(weights, X, Y):
    ...
    return binary_cross_entropy_loss(Y, predictions)
...
fname_train_acc = f'Results/Cross_Entropy_DataReuploading/train_acc_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_{seed}seed_Cross_Entropy_DataReuploading.txt'
...
fname_test_acc = f'Results/Cross_Entropy_DataReuploading/test_acc_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_{seed}seed_Cross_Entropy_DataReuploading.txt'
...
fname_train_cost = f'Results/Cross_Entropy_DataReuploading/train_cost_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_{seed}seed_Cross_Entropy_DataReuploading.txt'
...
fname_test_cost = f'Results/Cross_Entropy_DataReuploading/test_cost_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_{seed}seed_Cross_Entropy_DataReuploading.txt'
