...
for epoch in range(epochs):
    ...
    cost_train = binary_cross_entropy_loss(predictions_train, Y_train)
    ...
    cost_val = binary_cross_entropy_loss(predictions_val, Y_test)
    test_costs.append(cost_val)
    acc_val = accuracy(Y_test, np.sign(predictions_val))
    test_accs.append(acc_val)

fname_train_acc = f'Results/Cross_Entropy_DataReuploading/train_acc_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_CEDR.txt'
...
fname_test_acc = f'Results/Cross_Entropy_DataReuploading/test_acc_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_CEDR.txt'
...
fname_train_cost = f'Results/Cross_Entropy_DataReuploading/train_cost_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_CEDR.txt'
...
fname_test_cost = f'Results/Cross_Entropy_DataReuploading/test_cost_{layers}layers_lerningrate{lr}_batchsize{batch_size}_stocstep{stocastic_step}_{epochs}epochs_CEDR.txt'
