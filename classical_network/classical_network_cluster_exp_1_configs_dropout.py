...
history = model.fit(X_train, Y_train, epochs=20000, batch_size=64, validation_data=(X_test, Y_test), verbose=False, callbacks=[lr_scheduler, early_stopping])
...
fname_train_losses = f'Results/Res_dropout/Classical_network_{train_size}trainsize_trainloss_{seed}seed_{layers}layers_{n_nodes}nodes_Adam_Dropout.txt'
with open(fname_train_losses,'w') as f:
    for l in history.history['loss']:
        f.write(str(l))
        f.write('\n')

fname_test_losses = f'Results/Res_dropout/Classical_network_{train_size}trainsize_testloss_{seed}seed_{layers}layers_{n_nodes}nodes_Adam_Dropout.txt'
with open(fname_test_losses,'w') as f:
    for l in history.history['val_loss']:
        f.write(str(l))
        f.write('\n')

fname_train_accs = f'Results/Res_dropout/Classical_network_{train_size}trainsize_trainaccs_{seed}seed_{layers}layers_{n_nodes}nodes_Adam_Dropout.txt'
with open(fname_train_accs,'w') as f:
    for l in history.history['accuracy']:
        f.write(str(l))
        f.write('\n')

fname_test_accs = f'Results/Res_dropout/Classical_network_{train_size}trainsize_testaccs_{seed}seed_{layers}layers_{n_nodes}nodes_Adam_Dropout.txt'
