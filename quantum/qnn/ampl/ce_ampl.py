...
dev = qml.device('default.qubit.jax', wires=range(13))
...
while condition:
    ...
    if count_stoping == patience_stopiing:
        condition = False

    epoch += 1

fname_train_acc = f'Results/Cross_Entropy/train_acc_{layers}layers_batchsize{batch_size}_stocstep{stocastic_step}_{seed}seed_CE.txt'
...
fname_test_acc = f'Results/Cross_Entropy/test_acc_{layers}layers_batchsize{batch_size}_stocstep{stocastic_step}_{seed}seed_CE.txt'
...
fname_train_cost = f'Results/Cross_Entropy/train_cost_{layers}layers_batchsize{batch_size}_stocstep{stocastic_step}_{seed}seed_CE.txt'
...
fname_test_cost = f'Results/Cross_Entropy/test_cost_{layers}layers_batchsize{batch_size}_stocstep{stocastic_step}_{seed}seed_CE.txt'
